...
export const draw = (
  ...
  if (!isRunning) {
    ...
    if (!user.isHasLuckyDraw())
      return toast.error("Buy more articles to get a lucky draw");
    user.setAccount({
      ...user.account,
      luckyDraw: user.account.luckyDraw! - 1,
    });
    return start();
  }
  stop();
  if (randomReward) {
    ...
    if (randomReward.article) {
      toast.success(
        `Congratulation you got free ${randomReward.article} article`
      );
      user.isHasFreeArticles()
        ...
        : user.setAccount({
            ...user.account,
            freeArticles: randomReward.article,
          });
    }
  }
};
